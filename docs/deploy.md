...
**Releases** é o lugar certo para a versão: zip do código + notas. A tag `v*` dispara isso automaticamente (atual: `v0.9.0`).
...
9. Bling (opcional): webhooks `order` e `product` do aplicativo apontando para
   ...
   resposta, o aniversário). O [cron da Vercel](https://vercel.com/docs/cron-jobs)
   no plano gratuito só dispara uma vez por dia — use o cron do host, um
   `schedule` do GitHub Actions ou um plano pago.
...
1. GitHub → Releases → `v0.9.0` → Source code (zip)
...
- [ ] Migrações 001–091 aplicadas no Supabase de produção, **em ordem**. Algumas
      corrigem as anteriores (073 a 072, 079–081 os privilégios de funções, 087 a
      086, 090 a fila e as guardas da 085–089, 091 a 090) — pular uma deixa a seguinte sem o
...
- [ ] Cron **a cada minuto** com `AUTOMATION_CRON_SECRET` (senão as rotas respondem 503 e nada do funil anda)
