...
//
// An earlier implementation hard-defaulted to a marketing domain
// belonging to the upstream project, so an install that had not set
// `NEXT_PUBLIC_SITE_URL` sent invite links to somebody else's
// website, which 404s on `/join/<token>`. This resolution chain, and
// the localhost last resort at the bottom, remove that foot-gun.
...
function getBaseUrl(request: Request): string {
  ...
  if (allowList && (forwardedHost || host)) {
    ...
    console.warn(
      "[POST /api/account/invitations] could not derive base URL from request; falling back to localhost",
    );
  }
  // Localhost, and deliberately useless in production.
  //
  // This used to return `https://wacrm.tech` — the marketing site of
  // the upstream open-source project this codebase started from, a
  // domain nobody here controls, which 404s on `/join/<token>`. So the
  // failure mode of "the server could not work out its own address"
  // was an invite email carrying a working-looking link to a third
  // party's website.
  //
  // A localhost URL fails immediately and visibly, next to a warning
  // that says why. The fix is always the same and is now the only fix:
  // set `NEXT_PUBLIC_SITE_URL`, which is read first, above.
