...
export async function PATCH(
  ...
  // The rules of migration 065 — only the keys the caller sent.
  ...
  if (willBeActive) {
    ...
    warnings = collectActivationWarnings(mergedSteps);
  }

  if (Object.keys(update).length > 0 || Object.keys(ruleFields).length > 0) {
    let { error: updErr } = await admin
      .from('automations')
      .update({ ...update, ...ruleFields })
      .eq('id', id);
    // Pre-065 the rule columns do not exist; keep the rest of the edit.
    if (
      updErr &&
      Object.keys(ruleFields).length > 0 &&
      isUnknownColumn(updErr)
    ) {
