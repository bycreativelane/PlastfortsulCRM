...
export async function POST(request: Request) {
  ...
  if (body.template_slug) {
    const definition = getFlowTemplate(body.template_slug)
    if (!definition) {
      return NextResponse.json(
    ...
    // The clone is what the customer will actually talk to, so it is
    // seeded in the instance's language — not in the language the
    // template file happens to be written in.
    const catalogue = (await getMessages()) as unknown as {
      Flows?: { templates?: FlowTemplateCatalogue }
    }
    const template = localizeFlowTemplate(definition, catalogue?.Flows?.templates)
