...
describe('inbound webhook: marketing opt-out writes contacts.opted_out', () => {
  it('sets the flag on a standalone SAIR and does not fan out sends', async () => {
    await runWebhook({
      ...TEXT_MESSAGE,
      id: 'wamid.SAIR',
      text: { body: 'SAIR' },
    ...
    expect(h.state.contactUpdates).toEqual([
      expect.objectContaining({ opted_out: true }),
    ]);
    expect(h.dispatchInboundToFlows).not.toHaveBeenCalled();
    expect(h.dispatchInboundToAiReply).not.toHaveBeenCalled();
  ...
  it('sets the flag on the Não quero receber template button', async () => {
    await runWebhook({
      id: 'wamid.STOP',
      from: '15551230000',
      timestamp: '1700000000',
      type: 'button',
      button: { text: 'Não quero receber', payload: 'Não quero receber' },
    });
  ...
  it('does not treat a normal inbound as opt-out', async () => {
    await runWebhook();
