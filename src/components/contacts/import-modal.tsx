...
function PreviewCell({
  ...
  return (
    <span
      className={cn('block truncate', maxWidth, mono && 'text-2xs font-mono')}
      title={value}
    >
      {value}
    </span>
  ...
  return (
    <div className="flex min-w-[4.5rem] flex-wrap gap-1">
      {tagNames.map((name) => {
        ...
        return (
          // A tag that does not exist yet gets a dashed outline rather than a
          // slightly different alpha on a tinted border — the old distinction
          // was `55` vs `30` on the tag's own hex, which nobody can see.
          <TagChip
            key={name}
            color={color}
            size="sm"
            className={
              isKnown ? undefined : 'border-border border border-dashed'
            }
...
export function ImportModal({
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      {/* Not `max-h-vh-90`: this is a viewport height AND a pixel cap in one
      ...
      <DialogContent className="border-border/80 bg-popover text-popover-foreground flex max-h-[min(calc(90dvh/var(--zoom)),45rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
            ...
            <DialogDescription
              ...
              dangerouslySetInnerHTML={{
                __html: t.markup('desc', {
                  phoneCode: (chunks) =>
                    `<code class="rounded bg-muted px-1 py-0.5 text-2xs text-muted-foreground">${chunks}</code>`,
                  nameCode: (chunks) =>
                    `<code class="rounded bg-muted px-1 py-0.5 text-2xs text-muted-foreground">${chunks}</code>`,
                  emailCode: (chunks) =>
                    `<code class="rounded bg-muted px-1 py-0.5 text-2xs text-muted-foreground">${chunks}</code>`,
        ...
        {/* `empty:hidden` because this region is empty until a file is
            chosen: React renders the two false branches as nothing, so the
            element genuinely has no child nodes and `:empty` matches. Without
            it the dialog carried a 32px band of padding between the dropzone
            and the footer that read as a broken gap. */}
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4 empty:hidden">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {tagStats.rowsWithTags > 0 && (
                    // Was a hand-rolled span that reproduced `Tag`'s default
                    // geometry class for class — same chip, second source.
                    <TagChip>
                      <Tag className="text-muted-foreground mr-1 inline size-3 align-[-2px]" />
                      {t('previewTags', {
                  ...
                  <table className="w-full min-w-[32rem] text-xs">
                    ...
                    <tbody className="divide-border/70 divide-y">
                      {preview.map((row, i) => (
                        <tr
                          key={i}
                          className="bg-popover/40 hover:bg-muted/30 transition-colors duration-(--dur-1)"
                        >
          ...
          {result && (
            <div className="border-border bg-background/50 rounded-lg border p-4">
              ...
              {/* Two colours, not four. This summary lit up primary, ok,
                  human and danger at once, which is every signal the system
                  has in one 300px box — and amber on "skipped" promised an
                  action that does not exist: a duplicate the importer chose
                  not to re-create is information, not a task. Green is the
                  outcome that matters, red is the only one that failed, and
                  the rest carry their meaning in the icon. */}
              <div className="mt-3 flex flex-wrap gap-3">
                {result.imported > 0 && (
                  <div className="text-ok-ink flex items-center gap-1.5 text-sm font-medium">
                    <CheckCircle className="size-4 shrink-0" />
                    {t('resultImported', { count: result.imported })}
                  </div>
                )}
                {result.tagsAssigned > 0 && (
                  <div className="text-secondary-foreground flex items-center gap-1.5 text-sm">
                    <CheckCircle className="size-4 shrink-0" />
                ...
                {result.failed > 0 && (
                  <div className="text-danger-ink flex items-center gap-1.5 text-sm font-medium">
