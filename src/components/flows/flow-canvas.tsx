...
// Green / red branch ports for the condition node's true / false slots;
// every other slot inherits the node's own hue.
//
// These read the doctrine tokens directly rather than borrowing the
// start and handoff node hues, which is what they used to do. That
// worked only as long as handoff happened to be rose — and handoff is
// now amber, because it is the one node that calls a person. Borrowing
// a hue for its colour rather than its meaning is how a palette change
// silently repaints something unrelated.
...
function FlowNodeCard({ data, selected }: NodeProps) {
  ...
  return (
    <div
      ...
      className={cn(
        // `--lift-shadow` and not a hand-written `rgba(0,0,0,0.18)`.
        // That literal was tuned on the dark canvas; on the light one it
        // was ~3x the elevation every other surface in the app carries,
        // so a flow of twenty nodes read as twenty raised objects. The
        // token is defined per mode — a shadow in light, a hairline in
        // dark, where there is nothing darker to cast onto.
        'bg-card relative max-w-[260px] min-w-[220px] rounded-lg border px-3.5 py-3 text-left shadow-[var(--lift-shadow)] transition-[box-shadow,border-color] duration-(--dur-1)',
        selected
          ? 'border-[var(--nc)]'
          : 'border-border hover:border-[var(--nc-ring)]',
        // Flash on the accent, not amber. The comment here used to claim
        // it matched the list view; the list flashes `ring-primary`, and
        // amber is the app's one "a person must act" tone — spending it
        // on "here is the node you clicked in the validator" is exactly
        // the inflation that makes it stop meaning anything.
        isFlashed && '!border-primary ring-primary/60 ring-2'
      )}
    >
      {hasTarget && (
        <Handle
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="eyebrow truncate" style={{ color: c.text }}>
          {t(`nodes.${node.node_type}.label`)}
        </span>
        {isEntry && (
          <StatusBadge size="sm" className="ml-auto">
            {t('badgeEntry')}
          </StatusBadge>
        )}
      </div>
      <div className="text-muted-foreground text-2xs mt-2 truncate font-mono">
        {node.node_key}
      </div>
      {summary && (
      ...
      {isMultiSlot && (
        <div className="border-border mt-2.5 flex flex-col gap-1 border-t pt-2.5">
          {slots.map((slot) => (
            <div
              key={slot.id}
              className="text-muted-foreground text-2xs relative flex items-center justify-between gap-2 rounded-sm px-1 py-0.5"
            >
  ...
  if (!node) {
    return (
      <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
        <SheetContent side="right" size="panel" className="w-full" />
      </Sheet>
    );
  }
  const c = nodeColors(node.node_type);
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent
        side="right"
        size="panel"
        className="border-border bg-popover flex w-full flex-col gap-0 border-l p-0"
      >
        <SheetHeader className="border-border flex-row items-center gap-3 space-y-0 border-b px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <SheetTitle className="eyebrow flex items-center gap-2">
              <span style={{ color: c.text }}>
                {t(`nodes.${node.node_type}.label`)}
              </span>
              {isEntry && (
                <StatusBadge size="sm">{t('badgeEntry')}</StatusBadge>
              )}
            </SheetTitle>
          ...
          <code className="bg-muted text-muted-foreground text-3xs shrink-0 rounded-sm px-1.5 py-0.5 font-mono">
            {node.node_key}
          </code>
        </SheetHeader>
...
// list, same icons via NodeIconChip) but drops the new node into the
...
function CanvasAddNodeButton({ t }: { t: ReturnType<typeof useTranslations> }) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        // Raw button, so it misses the 44px hit shield globals.css
        // grants `[data-slot="button"]`; grown explicitly on touch.
        className="bg-primary text-primary-foreground hover:bg-primary-hover inline-flex min-h-9 items-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium shadow-[var(--lift-shadow)] transition-colors duration-(--dur-1) pointer-coarse:min-h-11"
        aria-label={t('addNode')}
      >
        <Plus className="h-4 w-4" />
        {t('addNode')}
      </DropdownMenuTrigger>
      <DropdownMenuContent align="start" className="w-[268px] p-1.5">
        {groupNodeTypesByCategory(ADD_NODE_TYPES).map((group, i) => (
          ...
          <Fragment key={group.id}>
            ...
            <DropdownMenuGroup>
              <DropdownMenuLabel className="text-muted-foreground eyebrow px-2 py-1.5">
