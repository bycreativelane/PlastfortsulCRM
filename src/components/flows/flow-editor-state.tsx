...
export function FlowEditorProvider({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      // The thrown message is an English HTTP string, not copy. It goes to
      // the console; the editor gets its own language.
  ...
  const setStatus = useCallback(
    async (next: BuilderState['status']) => {
      ...
      try {
        ...
        console.error('Flow status update failed:', err);
  ...
  const addNode = useCallback(
    (type: NodeType): string => {
      // Slugged from the TYPE, not the label.
      //
      // `NODE_META[type].label` is an English constant, so a node added on
      // a pt-BR install got a key like `handoff_to_agent` — an English
      // phrase printed on the canvas next to translated chrome, and one
      // that would have silently changed shape the day those labels were
      // translated. The type is already the stable snake_case identifier
      // this wants, and it is the same on every install.
