...
import { NodeIconChip, type BuilderNode } from "../shared";
        ...
        {options.map((n) => (
          <SelectItem key={n.node_key} value={n.node_key}>
            <span className="inline-flex items-center gap-1.5">
              {/* The chip, not a bare glyph: it is the same per-type
                  colour system the canvas and the list card use, and it
                  is the only one that holds contrast in both modes. */}
              <NodeIconChip
                type={n.node_type}
                size={18}
                iconSize={11}
