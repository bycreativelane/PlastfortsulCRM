            ...
            <div className="mb-2 flex items-center gap-2">
              ...
              {sections.length > 1 && (
                <Button
                  ...
                  className="text-danger-ink hover:bg-danger-soft shrink-0"
        ...
        <div className="md:col-span-2">
          ...
          {subject === 'tag' && tags.length > 0 ? (
            ...
            >
              ...
              <SelectContent>
                {/* The contact's own field NAMES, not the column
                    identifiers. This picker is read by whoever builds the
                    flow, and "company" is a schema detail that happens to
                    be legible in English and in nothing else. */}
      ...
      if (file.size > MEDIA_MAX_BYTES) {
        toast.error(
          t('toastFileTooLarge', {
            ...
            limit: Math.round(MEDIA_MAX_BYTES / 1024 / 1024),
          })
        );
        return;
      }
      ...
      <div>
        ...
        {cfg.media_url ? (
          <div className="border-border bg-muted flex items-center gap-2 rounded-md border px-3 py-2 text-xs">
            {/* Cyan is one of the raw Tailwind families globals.css does
                NOT re-point, so this clip and its hover both landed
                under 2:1 on the white card. The glyph is machine
                information (grey) and the link takes the accent. */}
            <Paperclip className="text-muted-foreground h-3.5 w-3.5 shrink-0" />
            <a
              href={cfg.media_url}
              target="_blank"
              rel="noopener noreferrer"
              className="text-foreground hover:text-primary min-w-0 flex-1 truncate transition-colors duration-(--dur-1) hover:underline"
            ...
            <button
              type="button"
              onClick={handleClear}
              // Raw <button>: `p-1` around a 14px glyph is a 22px target,
              // half the 44px the primitive gets on a coarse pointer.
              className="text-muted-foreground hover:bg-muted hover:text-foreground grid size-6 shrink-0 place-items-center rounded-sm transition-colors duration-(--dur-1) pointer-coarse:size-11"
          ...
          <button
            ...
            disabled={uploading}
            className="border-border bg-card text-muted-foreground hover:border-border hover:bg-muted hover:text-foreground flex w-full items-center justify-center gap-2 rounded-md border border-dashed px-3 py-4 text-xs transition-colors duration-(--dur-1) disabled:cursor-not-allowed disabled:opacity-60"
          >
            {uploading ? (
              <>
