...
/**
 * No `color` field here any more. It carried raw Tailwind families
 * (`text-sky-400`, `text-cyan-400`, `text-teal-400`, `text-fuchsia-400`)
 * that the theme never re-points, so on the white card they rendered
 * between 1.8:1 and 3.1:1 — the add-step menu's icons were effectively
 * one indistinguishable pale smear. `nodeColors()` below is the system
 * that already solves this in both modes; the two call sites that still
 * read `color` now go through `<NodeIconChip>`.
 */
...
const NODE_HUE: Record<NodeType, { l: number; c: number; h: number }> = {
  ...
  // condition and handoff used to be the other way round: amber on the
  // branch the engine evaluates by itself, rose (the failure family) on
  // the one node whose entire purpose is to hand the conversation to a
  // person. Amber is the app's only "come here", so it belongs on the
  // handoff and nowhere else on this canvas; a branch is machine work
  // and gets a cool neutral.
  condition: { l: 0.62, c: 0.05, h: 260 }, // cool neutral — machine decision
  set_tag: { l: 0.65, c: 0.15, h: 350 }, // pink
  handoff: { l: 0.72, c: 0.15, h: 65 }, // amber — the one node that calls a person
