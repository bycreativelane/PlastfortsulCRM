  ...
  return (
    <Banner tone="auto">
      <div className="flex min-w-0 flex-1 items-center gap-1.5">
        <Sparkles className="size-3.5 flex-shrink-0 text-auto-ink" />
  ...
  return (
    <div
      className={cn(
        ...
        // The bot replying on its own is the machine reporting, so it wears
        // `auto`. It used to wear a primary tint, which reads as a signal in
        // a system where the only signal is amber and primary is an accent —
        // and this banner sits above every automated thread all day.
        tone === "auto"
          ? "border-auto-border bg-auto-soft"
...
function BannerButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={busy}
      // "Take over" / "Resume AI" is the only control on this banner and a
      // raw <button> at ~26px is a comfortable click and a missed tap. The
      // height follows the pointer rather than the width, the same way the
      // conversation list's search and filter do.
      className="inline-flex h-6 flex-shrink-0 items-center gap-1 rounded-md border border-border bg-card px-2.5 font-medium text-foreground transition-colors duration-(--dur-1) hover:bg-muted disabled:opacity-60 [@media(pointer:coarse)]:h-11"
    >
      {busy ? (
