...
const MEDIA_MAX_HEIGHT = 'max-h-vh-75';
...
export function MediaLightbox({
  ...
  const timestamp = format(new Date(item.createdAt), 'PP HH:mm', {
    locale: dateLocale,
  });
        ...
        <DialogHeader className="flex-row items-start gap-3 pr-9">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <a
              ...
              className="text-muted-foreground hover:bg-muted hover:text-foreground flex size-8 items-center justify-center rounded-md transition-colors duration-(--dur-1) [@media(pointer:coarse)]:size-11"
            >
...
function ToolbarButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      // Raw <button>, so it gets none of the Button primitive's coarse-pointer
      // padding. Grown rather than shielded: these sit on a `gap-1` row, and
      // 44px hit shields around 32px drawings would overlap each other and
      // hand the tap to the neighbour.
      className="text-muted-foreground hover:bg-muted hover:text-foreground flex size-8 items-center justify-center rounded-md transition-colors duration-(--dur-1) disabled:opacity-60 [@media(pointer:coarse)]:size-11"
    >
      {busy ? (
...
function NavButton({
  ...
  return (
    <button
      ...
      title={label}
      className={cn(
        'border-border/60 bg-background/85 text-foreground hover:bg-background absolute top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-full border shadow-md backdrop-blur-sm transition-colors duration-(--dur-1) disabled:pointer-events-none disabled:opacity-0 [@media(pointer:coarse)]:size-11',
