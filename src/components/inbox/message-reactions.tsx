    ...
    <div className="mt-1 flex flex-wrap gap-1">
      {groups.map((g) => (
        <button
          ...
          className={cn(
            // h-5: the system has two chip heights and this one was drifting
            // to whatever the emoji's line box happened to be, so a row of
            // reactions never shared a baseline with anything beside it.
            "inline-flex h-5 items-center gap-1 rounded-full border px-1.5 text-2xs leading-none transition-colors duration-(--dur-1)",
            g.byCurrentUser
