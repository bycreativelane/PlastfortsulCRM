  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our PlastfortSul account',
      });
      onCreated();
    } catch (err) {
        ...
        {result ? (
          <>
            ...
            <div className="space-y-3 py-2">
              <FieldLabel>{t('inviteLink')}</FieldLabel>
              ...
              {/* "Copy this now, you will not see it again" is a genuine
                  call to act, so it keeps the amber — but as the audited
                  human- pair rather than an alpha wash. The old comment
                  reasoned about contrast "against slate-900"; the product
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The app's Button is the Base UI
