...
export function SettingsOverview({
  ...
  const tiles: {
  ...
  }[] = [
    {
      ...
      subtitle: !whatsapp?.configured ? (
        ...
        <>
          <StatusDot variant="ok" /> {t('connected')}
        </>
      ) : (
        <>
          <StatusDot variant="idle" /> {t('needsReconnecting')}
        </>
      ),
    },
    {
  ...
  return (
    <section className="animate-in fade-in-50 duration-(--dur-3)">
      {/* Identity. `flex` is explicit here: shadcn's Card shipped
          `flex flex-col` and this usage only overrode the direction. Panel is
          a plain bordered box, so without it the avatar stacks above the name
      ...
      <Panel className="flex items-center gap-4 p-4">
        <Avatar size="lg" className="size-14">
          ...
          <AvatarFallback seed={displayName} className="text-xl">
            {initial}
          </AvatarFallback>
        </Avatar>
        ...
        {tiles.map(({ section, loading, subtitle }) => {
          ...
          return (
            <button
              ...
              className={cn(
                'surface-interactive group border-border bg-card flex items-start gap-3.5 rounded-lg border p-4 text-left',
                'hover:bg-card-2'
              )}
            >
              ...
              <ChevronRight className="text-muted-foreground size-4 shrink-0 transition-transform duration-(--dur-2) group-hover:translate-x-0.5" />
            </button>
          );
        })}
      </div>
