...
//
// Three local adaptations vs the upstream file:
...
//   3. The axis tick classes are passed through `tick`, not
//      through the axis's own `className`. Upstream is written
//      against recharts ^2.13, where the ticks are children of
//      the axis group and inherit `font-size` from it. In
//      recharts 3 they are hoisted into a sibling
//      `recharts-zIndex-layer_2000` group and inherit nothing —
//      measured here, the axis <g> computed to 12px with the
//      class on it while the tick <text> inside computed to
//      16px, fill rgb(0, 0, 0). Every axis label in this chart
//      was rendering as browser-default black body copy.
//      See `components/charts/chart-primitives.tsx`, which has
//      the same note and the same fix for the charts on
//      /relatórios.
//
    ...
    return (
      ...
      >
        <div className={cx('border-b border-inherit px-4 py-2')}>
          <p className={cx('font-medium', 'text-popover-foreground')}>
            {label}
          </p>
        </div>
        ...
        <ResponsiveContainer>
          ...
          >
            ...
            <Tooltip
              ...
              // Was #d1d5db — a fixed light grey, which is a hover band you
              // cannot see on a dark card. --muted-foreground resolves per
              // mode, so the band stays a faint wash of the surface's own ink.
              cursor={{ fill: 'var(--muted-foreground)', opacity: '0.15' }}
              offset={20}
              position={{
