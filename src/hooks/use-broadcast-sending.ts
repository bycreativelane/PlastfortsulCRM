...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function resolveAudience(audience: AudienceConfig): Promise<Contact[]> {
    ...
    // Opt-out. Last, unconditional, and applied to EVERY audience type —
    // including CSV, which the exclude-tag filter above deliberately skips.
    //
    // That difference is the point. A tag exclusion is a targeting choice, so
    // it is reasonable for a hand-uploaded list to bypass it. Opt-out is not
    // a targeting choice: somebody told us to stop. And a CSV is exactly how
    // an opted-out number gets back into a send — pasted from a spreadsheet
    // that predates the request. By this line the CSV rows have been upserted
    // into `contacts`, so they carry the flag like everyone else.
    //
    // `!== true` rather than truthiness: migration 040 made the column NOT
    // NULL DEFAULT false, so the value is always a real boolean and this is
    // exact. It stays written defensively because `Contact.opted_out` is
    // optional in TypeScript — a caller that ever hands us a row from a
    // narrower select would otherwise silently pass everyone through.
    return contacts.filter((c) => c.opted_out !== true);
  }
