...
type ContactRow = { id: string; phone: string; opted_out: boolean | null }

async function loadSendableContact(
  ...
  // The inbound webhook sets this flag. A wait step can enqueue a send
  // days earlier; resume must still refuse. Flows do not share this
  // gate — a customer navigating a menu inside the 24h window they
  // opened is not the same as a marketing template after they asked
  // to stop.
...
export async function engineSendInteractive(
  ...
  await loadSendableContact(db, accountId, contactId)
...
async function sendViaMeta(input: SendInput): Promise<{ whatsapp_message_id: string }> {
  const db = supabaseAdmin()
  const contact = await loadSendableContact(db, input.accountId, input.contactId)
